...
const TASKS = [
  { id: "qa", label: "Q&A", desc: "General questions" },
  {
    id: "risk_analysis",
    label: "Risk Analysis",
    desc: "Monthly operational report",
  },
  ...
  const send = async (q) => {
    ...
    try {
      const res = await fetch(`${API_URL}/ask`, {
        ...
        body: JSON.stringify({ question, task, temperature: 0.3 }),
      });
      ...
      setMessages((m) => [
        ...m,
        {
          role: "assistant",
          error: err,
          text: "⚠️ Could not reach the API. Check your backend is running.",
        },
      ]);
    }
    setLoading(false);
  ...
  return (
    <div className="pt-24 pb-6 px-6 max-w-7xl mx-auto">
      ...
      <p className="text-[#8aad94] text-sm mb-6">
        Query the LLM analyst — grounded in BDH model output and SSE corporate
        documents
      </p>
